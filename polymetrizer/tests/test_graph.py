...
class TestAtomGraph:
    ...
    def pro_atom_graph(self, pro):
        return pro.graph.atomgraph_from_indices([1, 2, 3, 4])
    ...
    def test_hash_depends_atom_order(self, pro_atom_graph, pro):
        ...
        backwards = pro.graph.atomgraph_from_indices([4, 3, 2, 1])
        assert hashed == hash(backwards)

        jumbled = pro.graph.atomgraph_from_indices([1, 3, 2, 4])
    ...
    def test_hash_depends_edges(self, pro_atom_graph, pro):
        ...
        copy_atom_graph = copy_graph.atomgraph_from_indices([1, 2, 3, 4])
        assert hash(copy_atom_graph) != hash(pro_atom_graph)

        # check atom order again
        backwards = copy_graph.atomgraph_from_indices([4, 3, 2, 1])
        assert hash(copy_atom_graph) == hash(backwards)

        jumbled = copy_graph.atomgraph_from_indices([4, 2, 3, 1])
    ...
    def test_create_from_moleculargraph(self, cys):
        ...
        atom_graph = cys.graph.atomgraph_from_indices([0, 1, 2])
        ...
        rearranged = cys.graph.atomgraph_from_indices([1, 2, 0])
