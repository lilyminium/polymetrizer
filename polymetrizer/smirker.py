...
def create_full_atom_smirks(info):
    FULL_SMIRKS = ("[#{atomic_number:d}{aromaticity}H{hydrogen_count:d}"
                   "X{connectivity:d}x{ring_connectivity:d}{ring_size}"
                   "{formal_charge:+d}{label}]")
...
def create_smirks(oligomer, atom_indices=[], label_indices=[],
                  compressed=True):
    atom_chemper_info = get_mol_chemper_info(oligomer)
    raw_smarts = create_labeled_smarts(oligomer.offmol,
    ...
    # do a double pass
    for num in re.findall(r"\[[0-9a-zA-Z#@]*:([0-9]+)]", raw_smarts):
        new_pattern = r"(\[[0-9a-zA-Z#@]*:)(" + num + ")]"
        atom_smirks = r"\1-\2]"
        smirks = re.sub(new_pattern, atom_smirks, smirks)

    for num in re.findall(r"\[[0-9a-zA-Z#@]*:(-[0-9]+)]", smirks):
        index = (-int(num)) - 1
...
class SingleParameter:
    ...
    def __repr__(self):
        ...
        return (f"{clsname}(atom_indices={self.atom_indices}, "
                f"oligomer={self.oligomer}, parameter={self.parameter})")

    def __eq__(self, other):
        self_val = (self.atom_indices, self.oligomer, self.parameter)
        other_val = (other.atom_indices, other.oligomer, other.parameter)
        return self_val == other_val

    def create_smirks(
        self,
        context: Literal["all", "residue", "minimal"] = "residue",
...
class Smirker:
    ...
    def __init__(self, parameters_by_monomer_atoms={}, handler_name=None):
        ...
        self._is_singular_atom_term = all(len(p.atom_indices) == 1
                                          for p in self.single_parameters)

    def get_unified_smirks_parameters(
        self,
        context: Literal["all", "residue", "minimal"] = "residue",
        compressed: bool = True,
    ):
    ...
    def _unify_smirks(smirks_to_param):
        ...
        for smirks, params in smirks_to_param.items():
            err = ("I don't know how to deal with identical smirks "
                   "for different parameters yet")
    ...
    def get_smirks_for_oligomer(self, oligomer, compressed=True):
        ...
        for monomer_atoms, group_parameter in self.parameters_by_monomer_atoms.items():
            ...
            _, label_list = oligomer.contains_monomer_atoms(monomer_atoms,
                                                            handler_name=self.handler_name,
                                                            return_indices=True)
            for label_indices in label_list:
                tmp_parameter = SingleParameter(label_indices, oligomer, avg)
                smirks = tmp_parameter.create_smirks(context="all",
                                                     compressed=compressed)
    ...
    def get_combined_smirks_parameter(self, oligomer, compressed=True):
        if not self._is_singular_atom_term:
            err = "Combined SMIRKS is only supported for 1-atom terms"
        ...
        all_indices = [i for i, atom in enumerate(oligomer.offmol.atoms)
                       if atom.atomic_number != 0]
        smirks = create_smirks(oligomer, atom_indices=all_indices,
                               label_indices=label_indices,
                               compressed=compressed)
        ...
        for k, v in parameters.items():
            ...
            if not len(flat[k]) == n_indices:
                err = ("length mismatch between parameters and indices: "
                       f"len(indices) == {n_indices}, "
                       f"len({k}) == {len(flat[k])}")
